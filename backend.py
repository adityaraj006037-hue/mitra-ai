"""
MITRA Backend v4.0 — Supabase Persistent Storage
- All spaces, documents, chunks saved to Supabase
- Survives Render restarts
- ChromaDB rebuilt from Supabase on startup
"""
...
# APP
# =============================================================================
app = FastAPI(title="MITRA API", version="4.0.0")
...
# =============================================================================
# CHROMADB (in-memory, rebuilt from Supabase on startup)
# =============================================================================
...
async def supa_post(path: str, data: dict):
    async with httpx.AsyncClient(timeout=30.0) as client:
...
async def supa_patch(path: str, params: str, data: dict):
    async with httpx.AsyncClient(timeout=30.0) as client:
...
# =============================================================================
# STARTUP — rebuild ChromaDB from Supabase
# =============================================================================
@app.on_event("startup")
async def rebuild_chroma():
    """On startup, load all chunks from Supabase into ChromaDB."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        print("No Supabase config — running in memory-only mode")
        return
    try:
        chunks = await supa_get("chunks", "?select=*&limit=10000")
        if not isinstance(chunks, list):
            print("No chunks to restore")
            return
        
        # Group chunks by space
        by_space = {}
        for chunk in chunks:
        ...
        print(f"Restored {len(chunks)} chunks from Supabase across {len(by_space)} spaces")
    except Exception as e:
...
async def embed_document(space_id: str, doc_id: str, text: str, filename: str):
    ...
    if SUPABASE_URL and SUPABASE_KEY:
        try:
            ...
            print(f"Supabase chunk save error: {e}")
...
@app.get("/")
def root():
    return {"status": "MITRA API running", "version": "4.0.0"}

@app.get("/health")
async def health():
    # Load spaces from Supabase for accurate count
    count = 0
    if SUPABASE_URL and SUPABASE_KEY:
        try:
            ...
            pass
    return {"status": "ok", "version": "4.0.0", "spaces": count, "supabase": bool(SUPABASE_URL)}

# =============================================================================
# ROUTES — SPACES
# =============================================================================
@app.post("/spaces/create")
async def create_space(name: str = Form(...)):
...
async def list_spaces():
    if SUPABASE_URL and SUPABASE_KEY:
        try:
            spaces = await supa_get("spaces", "?select=*,documents(*)")
...
async def get_space(space_id: str):
    if SUPABASE_URL and SUPABASE_KEY:
        try:
            spaces = await supa_get("spaces", f"?id=eq.{space_id}&select=*,documents(*)")
            if not spaces or not isinstance(spaces, list):
                raise HTTPException(status_code=404, detail="Space not found")
            s = spaces[0]
            return {"id": s["id"], "name": s["name"], "docs": s.get("documents", []) or []}
...
async def delete_document(space_id: str, doc_id: str):
    collection = get_or_create_collection(space_id)
    try:
...
async def chat_with_space(
    ...
    try:
        ...
        if SUPABASE_URL and SUPABASE_KEY:
            try:
                await supa_patch("spaces", f"?id=eq.{space_id}", {"message_count": 999})
            except:
                pass
...
async def get_summary(space_id: str):
    return await chat_with_space(space_id, message="Give me a complete structured summary", history=[], mode="summary")

@app.post("/spaces/{space_id}/quiz")
async def get_quiz(space_id: str):
    return await chat_with_space(space_id, message="Generate 5 quiz questions with answers", history=[], mode="quiz")

@app.post("/spaces/{space_id}/mindmap")
async def get_mindmap(space_id: str):
    return await chat_with_space(space_id, message="Create a detailed mind map", history=[], mode="mindmap")

@app.post("/spaces/{space_id}/flashcards")
async def get_flashcards(space_id: str):
    return await chat_with_space(space_id, message="Generate 15 flashcards", history=[], mode="flashcard")

@app.post("/spaces/{space_id}/studyplan")
async def get_studyplan(space_id: str):
    return await chat_with_space(space_id, message="Create a 30-day study plan", history=[], mode="studyplan")
